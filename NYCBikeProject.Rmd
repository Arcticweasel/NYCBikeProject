...
# Mögliche Variante für missing Values beim Wetter (eventuell rausnehmen? Haben wir nicht genutzt)
...
#Erstellen Testdatensatz, da Abfragen sonst zu lang dauern (eventuell rausnehmen? Nutzen wir nicht)
...
```{r}
weather <- readRDS("Data/weather_rdy.rds")
```
...
```{r}
hourly_weather <- read_rds("Data/hourly_weather.RDS")
```
...
# load a map of nyc, first lets look for the zipcodes

lookup_code("New York", "New York")

# load the different borrows of new york into nyc_tracts with tigris
...
df$boroughCode[df$start_station_id==160 & is.na(df$boroughCode)] <- 1
df$borough[df$start_station_id==160 & is.na(df$borough)] <- "Manhattan"

df$neighborhood <- fct_explicit_na(df$neighborhood)
df$boroughCode <- fct_explicit_na(df$boroughCode)
df$borough <- fct_explicit_na(df$borough)


hourly_starts_nh <- df %>%
  group_by(start_date, start_hour, neighborhood) %>%
  summarise(user_count = n()) %>%
  mutate(weekend = ifelse(wday(start_date, week_start = 1) == 6 | wday(start_date, week_start = 1) == 7, 1,0))

hourly_starts_nh <- hourly_starts_nh %>%
  mutate(weekday = wday(start_date, abbr = F, label = T))

hourly_starts_nh$holiday <- ifelse(isBizday(as.timeDate(hourly_starts_nh$start_date), holidayNYSE(2016))==FALSE,1,0)

hourly_starts_nh <- hourly_starts_nh %>%
  full_join(hourly_weather, by = c("start_date" = "date", "start_hour" = "hour"))
...
hourly_starts_nh <- hourly_starts_nh[complete.cases(hourly_starts_nh[,3:7]),]#wetterdaten vom 23.-26.01 entfernen

apply(hourly_starts_nh, 2, function(x) sum(is.na(x)))#1585 stunden/neighborhoods fehlen werte->interpolieren

hourly_starts_nh[,8:15] <- na_interpolation(hourly_starts_nh[,8:15], option = "spline")

hourly_starts_nh <- dummy_cols(hourly_starts_nh, c("neighborhood", "weekday"), remove_first_dummy = T, remove_selected_columns = T)


```
```{r}
library(glmnet)
q1_nh <- hourly_starts_nh
...
# Setting alpha = 1 implements lasso regression, cross validation for opt. lambda
lasso_reg <- cv.glmnet(x, y, alpha = 1, lambda = lambdas, nfolds = 3)
