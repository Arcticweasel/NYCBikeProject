...
is.na(weather$precipitation) <- which(weather$precipitation == 'T')
is.na(weather$snow.fall) <- which(weather$snow.fall == 'T')
is.na(weather$snow.depth) <- which(weather$snow.depth == 'T')

weather <- na_interpolation(weather, option = "spline")
```
...
Einfluss auf etwaige Analyseergebnisse hat.

```{r}
weather$date <- dmy(weather$date)

weather <- weather %>%
  mutate(maximum.temperature = (maximum.temperature-32)*5/9,
         minimum.temperature = (minimum.temperature-32)*5/9,
         average.temperature = (average.temperature-32)*5/9)
...
weather <- weather %>%
  mutate(precipitation = precipitation*25.4,
         snow.fall = snow.fall*25.4,
         snow.depth = snow.depth*25.4)
...
gender <- df %>%
  group_by(stop_date, stop_hour, end_station_id, gender) %>%
  summarise(user = n())
gender <- pivot_wider(gender, names_from = gender, values_from = user)

hourly_stops <- cbind(hourly_stops, gender)
hourly_stops$start_date1 <- NULL
hourly_stops$start_hour1 <- NULL
hourly_stops$start_station_id1 <- NULL

usertype <- df %>%
  group_by(stop_date, stop_hour, end_station_id, usertype) %>%
  summarise(user = n())
usertype <- pivot_wider(usertype, names_from = usertype, values_from = user)
...
bike_q2_starts <- bike_q2 %>%
  ...
  summarise(avr_age = 2016-mean(birth_year), avg_tripduration = mean(tripduration), anzahl = n() )
...
Beim WEtter fehlen ein paar WErte
```{r}
