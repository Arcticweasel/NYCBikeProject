...
Auch hier werden nun die Nutzerzahlen von Männern und Frauen betrachtet, um festzustellen, ob Unterschiede in der Nutzung bestehen.

```{r}
hourly_starts %>%
  group_by(start_hour) %>%
  summarise(male_mean = round(sum(male_user_count)/365),
            female_mean = round(sum(female_user_count)/365)) %>%
  ggplot() +
  geom_line(aes(start_hour, male_mean, colour = "cadetblue"), size = 0.75) +
  geom_line(aes(start_hour, female_mean, colour = "cadetblue4"), size = 0.75) +
  scale_colour_manual(name = "Gender", values = c("cadetblue" = "cadetblue", "cadetblue4" = "cadetblue4"),
                      labels = c("Male", "Female")) +
  theme_light() +
  labs(title = "Durchschnittliche Nutzeranzahl Pro Stunde Nach Geschlecht",
       y = "Durchschnittliche Nutzeranzahl", x = "Stunde")
```

Es ist zu sehen, dass das Muster aus der vorherigen Visualisierung für beide Geschlechter erhalten bleibt. Es ist jedoch ebenfalls zu sehen, dass deutlich weniger Frauen als Männer die Fahrräder nutzen. Dies stimmt mit de Beobachtungen der monatlichen Nutzerzahlen überein. Auch hier kann es empfehlenswert für den Anbieter und die Stadt New York sein Forschungen anzustrengen, die die Gründe hierfür erörtern. So können Frauen, die beispielsweise lieber das Taxi nutzen als das Rad, vermehrt und besser auf sie abgestimmt angesprochen werden.
